...
import model.components.map_encoder as map_encoder_module
...
class MultimodalGenerativeCVAE(object):
    ...
    def create_node_models(self):
        ...
        if self.hyperparams['use_map_encoding']:
            if self.node_type in self.hyperparams['map_encoder']:
                ...
                # HACK to use another map encoder
                if 'name' in me_params:
                    map_encoder_class = getattr(map_encoder_module, me_params['name'])
                    self.add_submodule(self.node_type + '/map_encoder',
                            model_if_absent=map_encoder_class(me_params))
                else:
                    self.add_submodule(self.node_type + '/map_encoder',
    ...
    def obtain_encoded_tensors(self,
        ...
        if self.node_type == 'VEHICLE':
            # HACK to send vehicle heading, only used for modified unicycle
            node_heading = inputs[:, -1, 6]
            initial_dynamics['heading'] = node_heading
