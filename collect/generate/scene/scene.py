...
class AbstractSceneConfig(ABC):
    ...
    def __init__(self, record_interval=None, pixels_per_m=None, node_type=None):
...
class OnlineConfig(AbstractSceneConfig):
    ...
    def __init__(
        self, record_interval=5, pixels_per_m=3, node_type=NodeTypeEnum(["VEHICLE"])
    ):
        super().__init__(
            record_interval=record_interval,
            pixels_per_m=pixels_per_m,
...
class SceneBuilder(ABC):
    ...
    def __init__(
        ...
        if self.is_online:
            ...
            self.__last_frame = (
                self.__first_frame
                + (self.__scene_config.scene_interval - 1)
    ...
    def __capture_agents_within_radius(self, frame_id):
        ...
        if self.__trajectory_data is None:
            ...
            self.__trajectory_data = pd.concat(
                (self.__trajectory_data, df), ignore_index=True
            )
    ...
    def capture_trajectory(self, frame):
        if (frame - self.__first_frame) % self.__scene_config.record_interval == 0:
            ...
            should_capture_at_frame = (
                True
                if self.is_online
                else frame_id < self.__scene_config.scene_interval
            )
            if should_capture_at_frame:
                # logging.debug(
                #     f"in SceneBuilder.capture_trajectory() frame_id = {frame_id}"
                # )
...
def points_to_2d_histogram(points, x_min, x_max, y_min, y_max, pixels_per_m):
    bins = [pixels_per_m * (x_max - x_min), pixels_per_m * (y_max - y_min)]
