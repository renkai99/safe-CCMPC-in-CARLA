...
class MidlevelAgent(AbstractDataCollector):
    ...
    def compute_obstacle_constraints_GMM(
        ...
        return GMMconstraints, vertices, A_union, b_union, OVconstraint #, data_save
    ...
    def do_highlevel_control(self, params, ovehicles, Tsh):
        """Decide parameters."""
        timeLimit = 600
        ...
        u_bar_repeated = cp.Constant(np.tile(u_bar[:nu*T], (N_select, 1)))
        U_delta = U_temp - u_bar_repeated 

        # State variables x, y, psi, v
        x = util.obj_matmul(U_delta, Gamma[:nx*T,:nu*T].T) + x_bar[:nx*T]
        ...
        if params.O > 0:
            ...
            (
                GMMconstraints, vertices, A_unions, b_unions, OVconstraint
        ...
        try:
            ...
            prob.solve(solver=cp.CPLEX, cplex_params=cplex_params, verbose=False)
    ...
    def __compute_prediction_controls(self, frame, Tsh):
        ...
        ctrl_result, timeout, error = self.do_highlevel_control(params, ovehicles, Tsh)
        ...
        # X_init = np.repeat(params.initial_state.world[None], params.N_select, axis=0)
        # X = np.concatenate((X_init[:, None], ctrl_result.X_star), axis=1)
        # # logging.info(f"X_star:{np.min(X[:,-1,:][:,1])}") # min (X_final(1)) 
        # # logging.info(f"X_star:{X[:,-1,:][:,1]}") # min (X_final(1)) 
        # inputIndex = np.argmax(X[:,-1,:][:,1])
        # self.planned_finalstate= X[inputIndex,-1,:]
    ...
    def run_step(self, frame, Tsh=6, control=None):
        ...
        if (frame - self.__first_frame) % self.__scene_config.record_interval == 0:
            ...
            if frame_id < self.__n_burn_interval:
                ...
                speeds, angles, timeout = self.__compute_prediction_controls(frame, Tsh)
