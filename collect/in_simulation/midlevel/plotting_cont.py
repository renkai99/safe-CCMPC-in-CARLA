...
class PlotSimulation(object):
    ...
    def plot_mcc_timestep(
        ...
        values = []
        for t in range(self.T_coin):
            row = [planned_control[(0, t, 0)], planned_control[(0, t, 1)]]
            values.append(row)
        coin_control = np.array(values)
        ...
        cont_trajectory = np.array(cont_trajectory)

        # cont_control = planned_control[:, (self.T_coin - 1) :]
        N_max = max(key[0] for key in planned_control.keys()) + 1
        T_max = max(key[1] for key in planned_control.keys()) + 1
        nu = max(key[2] for key in planned_control.keys()) + 1

        cont_control = [[[planned_control.get((n, t, i), None) 
                        for t in range(self.T_coin - 1, T_max)] 
        ...
        cont_control_array = np.array(cont_control)
        cont_a = cont_control_array[..., 0]
        cont_delta = cont_control_array[..., 1]

        n_select = max(cont_control_array.shape[0], cont_xy.shape[0])
