...
class MidlevelAgent(AbstractDataCollector):
    ...
    def compute_obstacle_constraints(
        ...
        if self.road_boundary_constraints:
            ...
            S_big = np.zeros((L, T), dtype=float)
    ...
    def do_highlevel_control(self, params, ovehicles, Tsh, shrinking):
        ...
        x = util.obj_matmul(Gamma[:nx*T,:nu*T], u_delta) + x_bar[:nx*T]
        X = x.reshape(T, nx)
        U = cp.reshape(u, (T, nu))
        ...
        if params.O > 0:
            ...
            logging.info(f"shrinking{shrinking}")
            if shrinking and apply_robust == True:
                ...
                (
                    ...
                    ovStateMean_tau_1,
                    ovStateCov_tau_1
                ) = self.compute_robust_constraints_GMM(
                    params, ovehicles, Delta2, Omicron, temp_x, eps_ura, segments, T
                )
            else:
                if shrinking:
                    logging.info(f"Shrinking horizon, T = {T}, robustification = {apply_robust}")
        ...
        if self.get_computeTime:
            ...
            if self.log_cplex:
                ...
                prob.solve(solver=cp.CPLEX, cplex_params={"timelimit": timeLimit}, verbose=False)
